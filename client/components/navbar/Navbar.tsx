...
//   // ฟังก์ชันสลับสถานะของเมนูผู้ใช้
//   const toggleUserMenu = () => {
//     setUserMenuOpen(!userMenuOpen);
//   };

//   // ฟังก์ชันปิดเมนูเมื่อคลิกนอกเมนู
//   const closeMenus = () => {
//     if (userMenuOpen) setUserMenuOpen(false);
//     if (sidebarOpen && window.innerWidth < 640) setSidebarOpen(false);
//   };
...
//                 </div>
//                 {userMenuOpen && (
//                   <div className="absolute right-0 mt-2 top-10 z-50 my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-sm shadow-sm dark:bg-gray-700 dark:divide-gray-600">
...
//                       </li>
//                       <li>
//                         <Link href="/earnings" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-600 dark:hover:text-white" role="menuitem">Earnings</Link>
//                       </li>
//                       <li>
//                         <Link href="/logout" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-600 dark:hover:text-white" role="menuitem">Sign out</Link>
//                       </li>
//                     </ul>
//                   </div>
//                 )}
...
//       {(userMenuOpen || (sidebarOpen && window.innerWidth < 640)) && (
//         <div 
...
//           <button
//             onClick={() => {
//               // เพิ่มโค้ดสำหรับการ logout ตรงนี้
...
import { stopTokenPing } from '@/utils/tokenRefresher';
...
export default function Navbar({ children }: { children: React.ReactNode }) {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [userMenuOpen, setUserMenuOpen] = useState(false);
  const { setLineProfile,getUser } = useAuth();
  const router = useRouter(); // ใช้ useRouter จาก next/navigation

  const { user, isLoading, logout } = useAuth();

  useEffect(() => {
    // Log user data to verify it persists across page refreshes
  ...
  // ปิดเมนูเมื่อคลิกนอกพื้นที่หรือเมื่อรีไซซ์หน้าต่าง
  useEffect(() => {
    const handleClickOutside = (event) => {
      const userMenu = document.getElementById('user-menu');
      const userButton = document.getElementById('user-menu-button');

      if (userMenuOpen && userMenu && !userMenu.contains(event.target) && !userButton.contains(event.target)) {
  ...
  return (
    <>
        ...
        <div className="px-3 py-3 lg:px-5 lg:pl-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center justify-start rtl:justify-end">
              ...
              <Link href="/" className="flex ms-2 md:me-24">
                <span className="self-center text-xl font-semibold sm:text-2xl whitespace-nowrap dark:text-white">Social URL</span>
              </Link>
            </div>
            <div className="flex items-center">
              <div className="flex items-center ms-3">
                ...
                {userMenuOpen && (
                  <div
                    id="user-menu"
                    className="absolute right-0 mt-2 top-10 z-50 my-4 text-base list-none bg-white divide-y divide-gray-100 rounded shadow-lg dark:bg-gray-700 dark:divide-gray-600"
                    role="menu"
                  ...
                  >
                    ...
                    <ul className="py-1" role="none">
                      ...
                      <li>
                        <button onClick={handleLogout} className="flex px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-600 dark:hover:text-white" role="menuitem">Sign out</button>
                      </li>
                    </ul>
                  </div>
                )}
      ...
      <aside
        id="sidebar"
        className={`fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform bg-white border-r border-gray-200 sm:translate-x-0 dark:bg-gray-800 dark:border-gray-700 ${sidebarOpen ? 'transform-none' : '-translate-x-full sm:translate-x-0'
          }`}
        aria-label="Sidebar"
      >
        <div className="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
            ...
            <li>
              <Link href="/settings" className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
                <svg className="w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M5 11.424V1a1 1 0 0 0-2 0v10.424a3.228 3.228 0 0 0 0 6.152V19a1 1 0 1 0 2 0v-1.424a3.228 3.228 0 0 0 0-6.152ZM19.25 14.5A3.243 3.243 0 0 0 17 11.424V1a1 1 0 0 0-2 0v10.424a3.227 3.227 0 0 0 0 6.152V19a1 1 0 1 0 2 0v-1.424a3.243 3.243 0 0 0 2.25-3.076Zm-8-9A3.243 3.243 0 0 0 9 2.424V1a1 1 0 0 0-2 0v1.424a3.228 3.228 0 0 0 0 6.152V19a1 1 0 1 0 2 0V8.576A3.243 3.243 0 0 0 11.25 5.5Z" />
                </svg>
      ...
      {/* Overlay for closing menus on mobile */}
      {sidebarOpen && (
        <div
          className="sm:hidden fixed inset-0 z-30 bg-gray-900 bg-opacity-50 transition-opacity"
